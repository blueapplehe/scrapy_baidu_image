...
class ImageSpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        for key,one in enumerate(arrs):
            ...
            if True:
                #抓取原图时，百度是先加载页面～～然后再延时执行js获取大图反正好坑，所以要设置获取页面后要等待下
                request=scrapy.Request(detail_url,callback=self.parseDetail,meta={'item':image_item,'wait_time':5})#meta传递额外参数
                yield request
            else:
                yield image_item
